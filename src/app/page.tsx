...
export default function HomePage() {
  ...
  const trendPoints = useMemo(() => {
    ...
    return TREND_POINTS.slice(-monthCount);
  }, [selectedView]);
  ...
  const balanceHeaders = balanceCadence === "Monthly" ? monthlyHeaders : WEEK_HEADERS;
  const balanceRows = useMemo<TableRow[]>(
    () =>
      ENDING_BALANCE_ROWS.map((row) => ({
        label: row.label,
        sublabel: row.sublabel,
        values: balanceCadence === "Monthly" ? row.monthly.slice(-monthlyHeaders.length) : row.weekly
      })),
    [balanceCadence, monthlyHeaders.length]
  );

  const activityHeaders = activityCadence === "Monthly" ? monthlyHeaders : WEEK_HEADERS;
  const activityRows = useMemo<TableRow[]>(
    () =>
      NET_ACTIVITY_ROWS.map((row) => ({
        label: row.label,
        sublabel: row.sublabel,
        values: activityCadence === "Monthly" ? row.monthly.slice(-monthlyHeaders.length) : row.weekly
      })),
    [activityCadence, monthlyHeaders.length]
  );
